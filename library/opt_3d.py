...
class OptCsv:
    ...
    def create_csv(self):
        ...
        csv = filename + ".csv"
        pdf = filename + ".pdf"
        result_path = "3d_result"
        with open(f"{PROJECT_ROOT}/data/3d/demo.csv", "r", encoding="utf-8") as f:
        ...
        with open(f"{self.path}/{result_path}/{csv}", "w+", encoding="utf-8") as f1:
            f1.write(content)
            print(f"生成文件：{csv}")
        if shutil.copy(f"{PROJECT_ROOT}/data/3d/demo.pdf", f"{self.path}/{result_path}/{pdf}"):
            print(f"生成文件：{pdf}")
        print("肺功能3d 上传中...")
        time.sleep(6)
        try:
            with open(f"{self.path}{self.back_path}{self.date}/{result_path}/{csv}", "r"):
                return "肺功能3d 上传文件成功，请查验数据解析"
        except:
            return f"肺功能3d 上传文件失败，请检查！"
